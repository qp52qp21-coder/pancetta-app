...
- **配色は3色のみ**
  ...
  - 例外は2つだけ。アプリアイコン（下記）と、新規仕込み画面の背景の絵
    （`pancettaArt()`。アイコンと同じ多色を18%の薄さで敷く）
- **新規仕込み画面のカードだけ `rgba(255,255,255,.78)` で透かしている**。
  背景の絵を見せるため。入力欄・プリセット・計算結果は不透明のままにすること
  （文字が乗る面まで透かすと読みにくくなる）
- **背景色は `html` だけに持たせる**。`body` にも敷くと、その不透明な背景が
  `z-index:-1` の背景の絵を覆って見えなくなる
