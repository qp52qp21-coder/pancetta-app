...
- **アプリアイコンは豚さんではなく、独立した「バッジ」デザイン**
  （2026-09-13〜。旧・豚さんアイコンは `icons-history/` に履歴として保存）。
  ワインレッドの丸バッジ＋金の縁取り＋パンチェッタ断面のうずまき（生成り色ベース）。
  アイコン専用の配色（ワインレッド `#8A2430`系、金 `#D4AF37`、生成り `#FBF1E3`系、
  背景の焦げ茶グラデーション）を使ってよく、本文の3色ルールは適用しない
- **Androidのホーム画面用に `purpose:"maskable"` のアイコンを必ず用意する**。
  無いとChromeが「白い丸の中に縮小して置く」ため図柄が小さくなる。
  maskable版は背景を四隅まで敷き詰め、バッジ本体を**中央66.7%の円**に収める。
  purpose:"any" 版はバッジを円の縁ギリギリまで広げ、丸くクロップされたときに
  余白（背景色の輪）が見えないようにする
